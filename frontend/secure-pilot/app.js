(function () {
  ...
  if (CONFIG.TRANSPORT === "gateway") { window.startGoodDeedGatewayView({root:app, config:CONFIG}); return; }
  const callbacks = new Map();
  const state = { loadError: "", sessionToken: sessionStorage.getItem("gd_session") || "", user: null, deeds: [], members: [], membersLoaded: false, issuedCredential: null, tab: "records", busy: false, loginRole: "student", liffReady: false, liffLoggedIn: false, liffError: "", liffPilotOnly: false };
  ...
  async function login(event) {
    ...
    } catch(error) { toast(error.message,"error"); button.disabled=false; button.textContent="เข้าสู่ระบบ"; }
  }
  ...
  async function loadDashboard() {
    try { const result=await api("listDeeds",{limit:250}); state.deeds=result.deeds || []; state.loadError=""; renderDashboard(); }
    catch(error) { state.loadError="โหลดข้อมูลไม่สำเร็จ ข้อมูลที่แสดงอาจยังไม่เป็นปัจจุบัน กรุณากดรีเฟรช"; toast(error.message,"error"); renderDashboard(); }
  }
