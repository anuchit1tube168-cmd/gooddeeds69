...
test('legacy raw read routes are routed and write routes are denied',()=>{const s=setup();for(const action of ['getStudents','getDeeds','getSettings']){const r=s.context.doGet({parameter:{action}});assert.notEqual(r.code,'AUTHENTICATED_GATEWAY_REQUIRED','read action '+action+' should be routed');}for(const action of ['setupFolders'])assert.equal(s.context.doGet({parameter:{action}}).code,'AUTHENTICATED_GATEWAY_REQUIRED');for(const action of ['approveDeed','submit_deed','bind_line','uploadImage','init_all_students'])assert.equal(s.context.doPost({postData:{contents:JSON.stringify({action})}}).code,'AUTHENTICATED_GATEWAY_REQUIRED');});
