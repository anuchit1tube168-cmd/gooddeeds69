...
test('emergency lockdown blocks legacy raw data/password/write routes while preserving signed adapter dispatch',()=>{const s=setup();for(const action of ['getStudents','getDeeds','getStudent','getPassword','changePassword','setupFolders'])assert.equal(s.context.doGet({parameter:{action}}).code,'EMERGENCY_LOCKDOWN');for(const action of ['approveDeed','submit_deed','bind_line','uploadImage','init_all_students','changePassword'])assert.equal(s.context.doPost({postData:{contents:JSON.stringify({action})}}).code,'EMERGENCY_LOCKDOWN');const signed=s.context.doPost(s.signed());assert.equal(signed.ok,true);});
