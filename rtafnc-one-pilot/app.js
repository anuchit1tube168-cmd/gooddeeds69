(function(){
  ...
  function renderLogin(){
    app.innerHTML=`<main class="login-wrap"><section class="login-card">
      <div class="logo-ring"><img src="../frontend/510903.jpg" alt="ตรา วพอ."></div>
      <h1>RTAFNC ONE</h1><p class="sub">All for One · One for All<br>ต้นแบบรวมบริการ วิทยาลัยพยาบาลทหารอากาศ</p>
      <form id="login-form">
      ...
      <div class="notice">Pilot นี้แยกจากหน้า Production เดิม ใช้ API/สิทธิ์เดิมเฉพาะส่วนที่พร้อมแล้ว และยังไม่เปลี่ยน LIFF Endpoint</div>
    </section></main>`;
  ...
  function renderHome(){
    ...
    app.innerHTML=`<div class="shell">
      <header class="topbar"><div class="wrap topbar-in"><div class="brand"><img src="../frontend/510903.jpg" alt="RTAFNC"><div><strong>RTAFNC ONE</strong><small>Digital Student & College Operating System</small></div></div><div class="pill">PILOT ${esc(C.PILOT_VERSION)}</div></div></header>
      <main class="wrap">
        <section class="hero"><h1>สวัสดี ${esc(name)}</h1><p>หนึ่งตัวตน · หนึ่งประตู · ทุกบริการของ วพอ.</p></section>
        <section class="grid">
          <div class="id-card"><div class="id-head"><div class="id-brand"><img src="../frontend/510903.jpg"><div><b>วิทยาลัยพยาบาลทหารอากาศ</b><span>AIR FORCE NURSING COLLEGE</span></div></div><div class="level">${esc(roleLabel(u.role))}</div></div><div class="id-body"><div class="avatar"><div class="avatar-inner">${esc(initial)}</div></div><div><div class="id-name">${esc(name)}</div><div class="id-meta">รหัส ${esc(sid)}<br>รุ่น/ชั้นปี ${esc(cohort)} · Identity เชื่อมฐาน Good Deed</div></div></div><div class="id-foot"><div class="hours"><strong>${hours.toLocaleString('th-TH')}</strong><small>ชั่วโมงความดีที่อนุมัติแล้ว</small></div><div class="qr-mock" title="Dynamic QR — Phase 2"></div></div></div>
          <div class="panel"><h2>สถานะวันนี้</h2><div class="status-grid"><div class="metric ok"><strong>${approved.length}</strong><span>อนุมัติแล้ว</span></div><div class="metric warn"><strong>${pending.length}</strong><span>รอตรวจ</span></div><div class="metric"><strong>${isStaff?'STAFF':'LIVE'}</strong><span>${isStaff?'สิทธิ์ผู้ปฏิบัติงาน':'เชื่อมข้อมูลจริง'}</span></div></div><div class="system-list" style="margin-top:12px"><div class="system-row"><div><b>Good Deed V2</b><small>Apps Script + Drive + Audit</small></div><span class="dot"></span></div><div class="system-row"><div><b>Telegram Operations</b><small>ช่องทางหลักสำหรับ Admin / Approval / Alert</small></div><span class="dot pending"></span></div><div class="system-row"><div><b>LINE Urgent & Health</b><small>รอตรวจ Provider/LIFF ก่อนสลับประตู</small></div><span class="dot pending"></span></div></div></div>
        </section>
        <section class="panel" style="margin-top:16px"><h2>บริการของฉัน</h2><div class="actions">
          ...
          <div class="action" data-soon="advisor"><span class="ico">💬</span><b>อาจารย์ที่ปรึกษา</b><small>Conversation + Follow-up</small></div>
          <div class="action" data-soon="health"><span class="ico">🫶</span><b>สุขภาพและดูแลใจ</b><small>Urgent escalation</small></div>
          <div class="action" data-soon="docs"><span class="ico">📄</span><b>เอกสารและลงนาม</b><small>PDF + e-Approval</small></div>
          <div class="action" data-soon="inbox"><span class="ico">🔔</span><b>ONE Inbox</b><small>ข่าว / งาน / การแจ้งเตือน</small></div>
          <a class="action" href="${esc(C.LINKS.CARD_SHOWCASE)}"><span class="ico">💳</span><b>Digital Card Lab</b><small>ต้นแบบการ์ดเดิม</small></a>
          <a class="action" href="${esc(C.CURRENT_LIFF_URL)}"><span class="ico">🟢</span><b>LINE LIFF เดิม</b><small>ยังไม่เปลี่ยน Endpoint</small></a>
        </div></section>
        <section class="grid" style="margin-top:16px"><div class="panel"><h2>Action Feed</h2><div class="feed">${recentHtml}<div class="feed-item"><div class="feed-ico">📣</div><div><b>RTAFNC ONE Pilot เริ่มประกอบระบบแล้ว</b><p>ระบบจริงจะรวม Announcement, Approval, Advisor และ Alert ใน Feed เดียว</p></div><span class="tag">System</span></div></div></div><div class="panel"><h2>หลักการเชื่อมต่อ</h2><div class="system-list"><div class="system-row"><div><b>Telegram = Primary Operations</b><small>งานทั่วไป อนุมัติ ประชาสัมพันธ์ และเจ้าหน้าที่</small></div><span class="dot"></span></div><div class="system-row"><div><b>LINE = Critical Channel</b><small>สุขภาพ สุขภาพจิต เหตุเร่งด่วน และ Flex Message สำคัญ</small></div><span class="dot pending"></span></div><div class="system-row"><div><b>Google Drive = Master Repository</b><small>รูป PDF หลักฐาน เอกสาร และข้อมูลเดิม</small></div><span class="dot"></span></div></div></div></section>
      </main>
    ...
    document.querySelectorAll('[data-soon]').forEach(x=>x.onclick=()=>toast('โมดูลนี้อยู่ใน Sprint 4 วัน — ยังไม่เปิดเขียนข้อมูลจริง'));
