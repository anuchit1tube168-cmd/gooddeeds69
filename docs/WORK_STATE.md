...
| **P0-13** | **API Routing & Submission Handler Unblocking**<br>- แก้ไข `backend/server.py`: ปลดบล็อก `do_POST` ที่เคยถูก hardcode คืนค่า 403 `AUTHENTICATED_GATEWAY_REQUIRED` ให้ส่งต่อไปยัง handler จริง (`legacy_post_unsupported`)<br>- แก้ไข `do_GET` ให้เรียกใช้ API endpoints จริง (`legacy_get_unsupported`) สำหรับ `/api/get_student`, `/api/get_deeds` ฯลฯ<br>- ขยายการรองรับชื่อฟิลด์วันที่ (`activityDate`, `event_date`, `deedDate`, `date`) ใน `validate_deed_submission`<br>- ทดสอบ End-to-End Submission & Approval ผ่าน API สำเร็จ 100% | ✅ PASS | `backend/server.py`<br>`docs/WORK_STATE.md` |
| **P0-14** | **Student Login Unblocking & Centralized Password Management**<br>- แก้ไข `backend/server.py`: ปลดบล็อกการเสิร์ฟไฟล์ `data/students_data.js`, `data/deeds_data.js`, `data/app_settings.js` และภาพหลักฐานใน `send_head()` จากเดิมติด 403 ทำให้เบราว์เซอร์ใหม่โหลดรายชื่อไม่ได้<br>- ปรับปรุง `frontend/app.js`: เพิ่ม Dynamic API Fallback (`/api/get_student`) หากไม่พบในเครื่อง<br>- เพิ่มระบบจัดการรหัสผ่านส่วนกลาง: เพิ่ม `/api/change_password` อัปเดตลง `students.json` และ `students_data.js` พร้อมกันทั้ง 2 โฟลเดอร์<br>- เพิ่มฟังก์ชันช่วยจำและเติมรหัสเริ่มต้นอัตโนมัติเมื่อกด "ลืมรหัสผ่าน?" ใน `frontend/index.html`<br>- เพิ่มช่องแก้ไขและปุ่ม "🔄 คืนค่ารหัส 7 หลัก" ในหน้าแก้ไขนักเรียนของอาจารย์ (`teacher-dashboard.html`)<br>- อัปเดต Cache-Busting Version เป็น `?v=3592` ครบทุกหน้าที่เกี่ยวข้อง | ✅ PASS | `backend/server.py`<br>`frontend/app.js`<br>`frontend/index.html`<br>`frontend/profile.html`<br>`frontend/teacher-dashboard.html` |
